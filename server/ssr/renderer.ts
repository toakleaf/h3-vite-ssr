          ...
          onAllReady() {
            ...
            if (isProd) {
              ...
              try {
                ...
                // Collect CSS from entry and its static imports recursively
                ...
                while (queue.length) {
                  ...
                  if (Array.isArray(m.css)) {
                    for (const href of m.css) cssHrefs.push('/' + href.replace(/^\/?/, ''))
                  }
                  const deps = [
                    ...(Array.isArray(m.imports) ? m.imports : []),
                    ...(Array.isArray(m.dynamicImports) ? m.dynamicImports : [])
                  ]
                  for (const dep of deps) if (!visited.has(dep)) queue.push(dep)
                }
              } catch {}
              for (const href of cssHrefs) event.node.res.write(`<link rel="stylesheet" href="${href}"/>`)
